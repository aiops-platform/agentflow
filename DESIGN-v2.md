# 基于 opencode 的 AI 运维 Bug Fix 智能体 Workflow — 技术选型与架构设计（v2）
...
- **permission.asked 自动 reply**：opencode 的工具级权限请求（如 `external_directory`，写操作 agent 访问 `/tmp` 时触发）由 adapter 自动 `POST /permission/:id/reply` 回复 `once`（批准单次），避免 session 卡死在审批等待超时（否则 fix/tester 会卡满 httpx 300s）。与 agentflow 自身的 `ApprovalManager`（业务级审批）是两层。
...
- **审批节点（`kind: approval`）**：除「节点属性式」审批（`approve` 字段）外，还支持「显式审批节点」——`NodeDef.kind=approval` 的节点不调 LLM，只做人工审批。`params` 声明展示/透传的直接上游 output（单上游透传单个、多上游透传 dict）；通过则原样透传、驳回则标 `rejected-canceled` 并停下游。审批节点可插在任意两个节点之间（如 `fix → approve-fix → test`），把审批做成流程的显式步骤。配套 `POST /runs/:id/approve` / `/reject` 端点 + `GET /runs/:id` 返回 `pending_approvals`（含上游 output），前端展示审批按钮。
...
**（v2）ConsoleSink（本地调试观测）**：除 Langfuse/OTel 外，新增第三个 sink `ConsoleSink`——CLI `run` 时实时打印每节点 `▶ 开始 → 输入 prompt + 输出 output → ✅/❌ 状态(tokens/cost/耗时)`，面向本地调试。只在 `cli.py` 订阅（`server.py` 不订阅，避免污染服务日志），与 Langfuse/OTel 的机器观测互补。
...
1. **每个 agent 的 output 强制双分层**：`summary`（≤500 token 核心结论）+ `details`（完整证据，只落 StateStore）。下游默认只拿 `summary`，需要细节时通过 MCP 工具 `read_upstream_output(node_id, field)` 按需拉取。
   - **（v2）`input_view` 的 None 字段裁剪**：summary 视图（`NodeDef.input_view=summary`，默认）不仅去掉 `details`，还去掉值为 `None` 的顶层字段——schema 校验后 evidence 类有大量 `None` 字段（如 `error_stack: null`），去掉后下游只拿有实际内容的 summary。节点级可覆盖，agent 级由 `spec.py` 声明。
...
**（v2）json5 宽松解析**：output 提取时标准 JSON 解析失败后，用 json5 兜底，容忍 LLM 输出的尾逗号/注释/单引号 key 等瑕疵，降低「模型输出不是合法 JSON」导致的节点失败。json5 是 fastmcp 的传递依赖，`try import` 惰性加载，不可用时跳过。
...
- **成本预算**：run 级 token/cost 上限，环境变量 `AGENTFLOW_MAX_COST` / `AGENTFLOW_MAX_TOKENS`（默认 `0` = 不限）；`DAGExecutor` 累计每节点 cost/tokens，超限触发 `abort` 停下游。
